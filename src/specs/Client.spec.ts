...
it("should output typescript from a database", async () => {
  ...
  const mockConvert = vi
    .mocked(DatabaseTasks.convertDatabaseToTypescript)
    .mockReturnValue("export interface Database {}")
  const mockGenerate = vi
    .mocked(DatabaseTasks.generateDatabase)
...
it("should fetch a database object with a provided db", async () => {
  ...
  const result = await Client.fromConfig(config)
    .fetchDatabase(mockDb as any)
    .toObject()
...
it("should apply mappings to all tables", async () => {
  ...
  expect(mockFn).toHaveBeenCalledWith(
    mockDatabase.schemas[0].tables[0],
    mockDatabase.schemas[0],
  )
  expect(mockFn).toHaveBeenCalledWith(
    mockDatabase.schemas[0].tables[1],
    mockDatabase.schemas[0],
  )
  expect(mockDb.destroy).toHaveBeenCalled()
  expect(mockGenerate).toHaveBeenCalledWith(appliedConfig, mockDb)
...
it("should apply mappings to all columns", async () => {
  ...
  expect(mockFn).toHaveBeenCalledWith(
    mockDatabase.schemas[0].tables[0].columns[0],
    mockDatabase.schemas[0].tables[0],
    mockDatabase.schemas[0],
  )
  expect(mockFn).toHaveBeenCalledWith(
    mockDatabase.schemas[0].tables[1].columns[0],
    mockDatabase.schemas[0].tables[1],
    mockDatabase.schemas[0],
  )
  expect(mockFn).toHaveBeenCalledWith(
    mockDatabase.schemas[1].tables[0].columns[0],
    mockDatabase.schemas[1].tables[0],
    mockDatabase.schemas[1],
  )
  expect(mockFn).toHaveBeenCalledWith(
    mockDatabase.schemas[1].tables[1].columns[0],
    mockDatabase.schemas[1].tables[1],
    mockDatabase.schemas[1],
  )
  expect(mockDb.destroy).toHaveBeenCalled()
  expect(mockGenerate).toHaveBeenCalledWith(appliedConfig, mockDb)
...
it("should apply mappings to specified column", async () => {
  ...
  const result = await Client.fromConfig(config)
    .fetchDatabase()
    .mapColumn("public.users.id", mockFn)
    .toObject()
