...
describe("getAllEnums", () => {
  it("should return all enums for a table sorted", async () => {
    ...
    vi.mocked(AdapterFactory.buildAdapter).mockReturnValue(
      mockAdapter as unknown as AdapterInterface,
    )
    const db = {
      client: {
        dialect: "dialect",
...
describe("generateEnumName", () => {
  it("should generate an enum name, removing invalid characters", () => {
    ...
    expect(EnumTasks.generateEnumName("name ;] 2", config as any)).toBe(
      "name2Entity",
    )
  })
})
...
describe("generateEnumKey", () => {
  ...
  it("should return the key exact if not a number", () => {
    ...
    expect(EnumTasks.generateEnumKey("notanumber", config as any)).toBe(
      "notanumber",
    )
  })
})
