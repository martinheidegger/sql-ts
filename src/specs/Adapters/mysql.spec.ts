...
beforeAll(async () => {
  ...
  const connectionString =
    sqlContainer.getConnectionUri().replace("test:", "root:") +
    "?tinyInt1isBit=true"
...
describe("getAllColumns", () => {
  it("should return all columns for the specified table and schema", async () => {
    ...
    const columns = await mysql.getAllColumns(
      db,
      config,
      "table_one",
      "schema_one",
...
describe("parseEnumString", () => {
  ...
  it("should parse various complex values", () => {
    expect(
      mysql.parseEnumString(
        `enum('ac''tive','inac,ti.ve',' pen"#d ing','valid')`,
      ),
    ).toEqual(["ac'tive", "inac,ti.ve", ' pen"#d ing', "valid"])
  })
})
