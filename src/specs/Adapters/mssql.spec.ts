...
beforeAll(async () => {
  ...
  const config = {
    client: "mssql",
    connection: ConnectionPoolMethods.parseConnectionString(
      sqlContainer.getConnectionUri(),
    ),
  }
...
describe("getAllTables", () => {
  it("should return all schemas if not specified", async () => {
    ...
    const ourTables = tables.filter(
      (t) => t.schema === "schema_one" || t.schema === "schema_two",
    )
...
describe("getAllColumns", () => {
  it("should return all columns", async () => {
    ...
    const columns = await mssql.getAllColumns(
      db,
      config,
      "table_one",
      "schema_one",
