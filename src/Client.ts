...
interface ClientWithDatabase {
  ...
  mapTable(
    identifer: string,
    func: (table: Table, schema: Schema) => Table,
  ): ClientWithDatabase
  /**
  ...
  mapColumn(
    identifier: string,
    func: (column: Column, table: Table, schema: Schema) => Column,
  ): ClientWithDatabase
  /**
...
export class Client implements ClientWithConfig, ClientWithDatabase {
  ...
  public fetchDatabase(db?: Knex): ClientWithDatabase {
    this.databaseProvider = async () => {
      if (db == null) {
        return await ConnectionFactory.createAndRun(this.config, (tempdb) =>
          DatabaseTasks.generateDatabase(this.config, tempdb),
        )
      }
    ...
    this.databaseMappings.push((database: Database) => {
      for (const schema of database.schemas) {
        schema.tables = schema.tables.map((table) =>
          `${schema.name}.${table.name}` === identifer
            ? func(table, schema)
            : table,
        )
      }
      ...
      for (const schema of database.schemas) {
        for (const table of schema.tables) {
          table.columns = table.columns.map((column) =>
            `${schema.name}.${table.name}.${column.name}` === identifier
              ? func(column, table, schema)
              : column,
          )
        }
      }
  ...
  public mapSchema(
    ...
    this.databaseMappings.push((database: Database) => {
      database.schemas = database.schemas.map((schema) =>
        schema.name === identifier ? func(schema) : schema,
      )
    })
    return this
  }
